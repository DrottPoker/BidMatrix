...
grant usage on schema better_auth to {{AUTH_ROLE}};
...
grant select, insert, update, delete on all tables in schema better_auth to {{AUTH_ROLE}};
...
grant execute on function public.inspect_tenant_owner_invitation(text, timestamptz)
    to {{AUTH_ROLE}};

alter role {{AUTH_ROLE}} set search_path = better_auth, public;
